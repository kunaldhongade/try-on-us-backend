...
const missingEnv = REQUIRED_ENV.filter((env) => !process.env[env] || process.env[env]?.includes("..."));
if (missingEnv.length > 0) {
    console.warn(`\x1b[33mWarning: Missing or placeholder environment variables: ${missingEnv.join(", ")}\x1b[0m`);
    console.warn(`\x1b[33mThe system will use mock data for these services until real keys are provided in .env\x1b[0m`);
}
const app = express();
...
app.post("/tryon", requireAuth, async (req, res) => {
    const { productId, variantId, shop, originalImageUrl, productTitle, personImageBase64, } = req.body;
        ...
        (async () => {
            try {
                const personBuffer = personImageBase64
                    ...
                    : Buffer.alloc(0);
                const result = await runVirtualTryOn(personBuffer, tryonEntry.originalImageUrl, productTitle || "garment");
...
app.get("/tryon-status/:id", async (req, res) => {
    try {
