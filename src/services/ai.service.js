...
export const runVirtualTryOn = async (personImage, garmentImageUrl, garmentDescription) => {
    try {
        if (!PROJECT_ID) {
            throw new Error("GCP_PROJECT_ID is not configured");
        }
        console.log(`Starting Vertex AI Pipeline (Project: ${PROJECT_ID}, Location: ${LOCATION})...`);
        ...
        if (!garmentResponse.ok) {
            throw new Error(`Failed to download garment image: ${garmentResponse.statusText}`);
        }
        ...
        if (!predictions || !predictions[0] || !predictions[0].bytesBase64Encoded) {
            console.error("Vertex AI result empty or invalid:", JSON.stringify(response.data, null, 2));
        ...
        const resultUrl = await uploadToS3(resultBuffer, `tryon-${Date.now()}.png`, "image/png");
        return { imageUrl: resultUrl, status: "done" };
    }
    catch (error) {
        console.error("AI Pipeline Error:", error.response?.data || error.message || error);
